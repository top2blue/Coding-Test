...
public class Ex05 {
	...
	class Solution {
		public int[] solution(int[] arr, int divisor) {
			...
			for(int n : arr) if(n%divisor==0) div.add(n); // 나누어지는 것만 리스트에 추가
			Collections.sort(div); // 정렬
			if(div.size()==0) { // 개수가 0이면 
				answer = new int[] {-1}; // 없다고 -1을 리턴
			}else {
				answer = new int[div.size()]; // 배열로 결과를 보내야 하므로 배열 선언
				for(int i=0;i<answer.length;i++) answer[i] = div.get(i); // 리스트를 배열로 복사
			}
			return answer; // 결과 리턴
		}
	}
