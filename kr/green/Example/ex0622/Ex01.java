...
public class Ex01 {
	...
	public void testSolution() {
		...
		assertArrayEquals(solution.solution(new int[]{1, 5, 2, 6, 3, 7, 4}, 
...
class Solution {
	public int[] solution(int[] array, int[][] commands) {
		int[] answer = new int[commands.length]; //정답은 두번째 배열의 행수
		for (int i = 0; i < commands.length; i++) { // 반복
			int[] ar = new int[commands[i][1] - commands[i][0] + 1]; // 잘라낼 배열의 크기
			// 배열 복사
