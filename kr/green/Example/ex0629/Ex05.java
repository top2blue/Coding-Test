...
옵션으로 스타상(*) , 아차상(#)이 존재하며 스타상(*) 당첨 시 해당 점수와 바로 전에 
...
public class Ex05 {
	...
	class Solution {
	    public int solution(String dartResult) {
	    	char[] darts = dartResult.toCharArray(); // char배열로 만듬
	        int[] score = new int[3]; // 점수는 3개
	        int cnt = 0;
	 
	        for(int i = 0; i < darts.length; i++) {
	            if(darts[i] >= '0' && darts[i] <= '9') { // 숫자라면
	                if(darts[i] == '1' && darts[i+1] == '0') {// 현재가 1이고 다음이 0이면 
	                    score[cnt] = 10; // 10점
	                    i++; // 다음 문자로 전진
	                } else {
	                    score[cnt] = darts[i] - '0'; // 숫자로 변경
	                }
	                cnt++;
	            } else if(darts[i] == 'D') { // D이면 숫자의 제곱
	                score[cnt] *= score[cnt];
	            } else if(darts[i] == 'T') { // T이면 숫자의 세제곱
	                score[cnt] *= score[cnt] * score[cnt];
	            } else if(darts[i] == '*') { // 스타상(*) 당첨 시 해당 점수와 바로 전에 
	            							 // 얻은 점수를 각 2배로 만든다
	                if(cnt > 0) {
	                    score[cnt-1] *= 2;
	                }
	                score[cnt] *= 2;
	            } else if(darts[i] == '#') { // 아차상(#) 당첨 시 해당 점수는 마이너스된다.
	                score[cnt] *= -1;
	            }
	        }
