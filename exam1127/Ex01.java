...
public class Ex01 {
	...
	public void test() {
		...
		assertFalse(solution.solution(new String[] {"1192","119"}));
	}
	class Solution {
	    public boolean solution(String[] phone_book) {
	        boolean answer = true;
	        for(int i=0;i<phone_book.length;i++) { // 문자열에 공백 제거
	        	phone_book[i] = phone_book[i].replaceAll("\\s", "");
	        }
	        ...
	        loop:for(int i=0;i<phone_book.length-1;i++) {
	        	for(int j=i+1;j<phone_book.length;j++) {
	        		if(phone_book[j].length()>phone_book[i].length() && phone_book[j].startsWith(phone_book[i])) {
	        			System.out.println(phone_book[i] + " : " + phone_book[j] + " = " + (phone_book[j].startsWith(phone_book[i])));
	        			answer = false;
	        			break loop;
	        		}
	        	}	   
